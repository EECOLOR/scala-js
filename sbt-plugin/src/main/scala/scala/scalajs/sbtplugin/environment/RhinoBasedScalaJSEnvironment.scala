...
import scala.scalajs.sbtplugin.environment.rhino.ContextOps
import scala.scalajs.sbtplugin.environment.rhino.ScriptableObjectOps
import scala.scalajs.sbtplugin.environment.rhino.ScalaJSCoreLib
...
class RhinoBasedScalaJSEnvironment(
   ...
   *
   * 1. The core lib (using lazy loading)
   * 2. The Scala.js files (on the classpath)
   * 3. Other js files and
   * 4. Scala.js files (not on the classpath)
   */
  lazy val (
    scalaJSCoreLib,
    scalaJSProviders,
    ...
    (scalaJSCoreLib, scalaJSProviders, availableJsFiles, scalaJSFiles, envJSLib)
  }

  /**
   *  Allows you to execute code in an environment where the Scala.js
   *  library is set up to lazily load it's classes.
   *
  ...
  private def importScripts(context: Context, globalScope: Scriptable, availableJsFiles: Map[String, File])(args: Array[AnyRef]) = {
    ...
    files.foreach {
      ...
      case (url, None) =>
        throw new RuntimeException("Could not find file: '" + url + "', make sure you make it available on any classpath")
    }
  }

  private class FileNameMatcher(pattern: Regex) {
    def unapply(file: File): Option[Unit] =
      file.getName match {
        case pattern(_*) => Some()
        case _ => None
      }
  ...
  private object RelativeScalaJSProvider {
    ...
    private def makeRelative(fileName: String, classpath: String) = {
      ...
      val relativeFileName =
        rel.substring(0, lastSlash + 1) + fileName
