...
trait Utilities { self: CodeBlock =>
  ...
  def getObject(base: String, className: String) = {
    ...
    classObject match {
      ...
      case Right(noFunction) =>
        throw new RuntimeException("Could not find constructor function, type of element was: " + noFunction.getClass.getName)
      case Left(key) =>
        throw new RuntimeException("Could not find $className, undefined key: " + key)
    }
  }
  ...
  def callMethod(obj: NativeObject, methodName: String, args: Any*) = {
    ...
    method match {
      ...
      case noFunction =>
        throw new RuntimeException(s"$methodName is not a method, type was: " + noFunction.getClass.getName)
    }
  }

  @tailrec
  private def getFrom(obj: Scriptable, keys: Seq[String], scope: Scriptable): Either[String, Scriptable] =
