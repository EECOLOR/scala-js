...
class TestOutputBridge(eventProxy: EventProxy) extends TestOutput {
  ...
  val errorColor: String = eventProxy.red()
  val successColor: String = eventProxy.green()
  val infoColor: String = eventProxy.blue()

  def removeColors(message: String): String =
    eventProxy.removeColors(message)

  def color(message: String, color: String): String =
    eventProxy.color(message, color)
